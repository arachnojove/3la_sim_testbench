...
void printInstruction(int num_insn, VTAGenericInsn *insns) {
  ...
  for (int i = 0; i < num_insn; i++) {
    ...
    uint64_t* temp = reinterpret_cast<uint64_t*>(&insns[i]);
    printf("[ILA-INFO]: instr No.%d \t %016llx%016llx\n", i, temp[1], temp[0]);
    c.generic = insns[i];
...
void printMicroOp(int num_uop, VTAUop *uops) {
  ...
  for (int i = 0; i < num_uop; i++) {
    // Read micro-op
    printf("[ILA-INFO]: uop No.%d \t info: %016llx\n", i, uops[i]);
...
int alu_test(int opcode, bool use_imm, int batch, int vector_size, bool uop_compression) {
  ...
  printf("=====================================================================================\n");
  printf("INFO [INSTR-START] - ALU test of %s: batch=%d, vector_size=%d, uop_compression=%d\n",
  ...
  for (int i = 0; i < (VTA_ACC_ELEM_BYTES * batch * tx_size * input_sets)/4; i++) {
      printf("[ILA-INFO]: alu_test bias_buf[%d]: %08llx\n", i, bias_buf[i]);
  }
  ...
  for (int i = 0; i < VTA_OUT_ELEM_BYTES * batch * tx_size * input_sets; i++ ) {
      uint8_t* temp_buf = reinterpret_cast<uint8_t*>(output_buf);
      printf("[ILA-INFO]: alu_test result out_buf[%d]: %08llx\n", i, temp_buf[i]);
  }
...
int blocked_gemm_test(int batch, int channels, int block, bool uop_compression,
  ...
  printf("=====================================================================================\n");
  printf("INFO [INSTR-START] - Blocked GEMM test: batch=%d, channels=%d, block=%d, uop_comp=%d, vt=%d\n",
  ...
  for (int i = 0; i < VTA_INP_ELEM_BYTES * inp_size; i++) {
      uint8_t* temp_buf = reinterpret_cast<uint8_t*>(input_buf);
      printf("[ILA-INFO] GEMM input_buffer[%d]: %02llx\n", i, temp_buf[i]);
  }
  ...
  for (int i = 0; i < VTA_WGT_ELEM_BYTES * wgt_size; i++) {
      uint8_t* temp_buf = reinterpret_cast<uint8_t*>(weight_buf);
      printf("[ILA-INFO] GEMM weight_buffer[%d]: %02llx\n", i, temp_buf[i]);
  }
  ...
  for (int i = 0; i < VTA_ACC_ELEM_BYTES * out_size/4; i++) {
      printf("[ILA-INFO] GEMM bias_buf[%d]: %08llx\n", i, bias_buf[i]);
  }
  ...
  for (int i = 0; i < VTA_INP_ELEM_BYTES * out_size; i++) {
	uint8_t* temp_buf = reinterpret_cast<uint8_t*>(output_buf);
	printf("[ILA-INFO] GEMM output_buf[%d]: %08llx\n", i, temp_buf[i]);
  }
  // Unpack output data
...
int gemm_test(int batch, int in_channels, int out_channels, bool uop_compression) {
  ...
  printf("=====================================================================================\n");
  printf("INFO [INSTR-START] - Blocked GEMM test: batch=%d, in_channels=%d, out_channels=%d, uop_comp=%d\n",
