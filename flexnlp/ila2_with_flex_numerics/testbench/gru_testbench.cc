...
//  void source_input();
  ...
  void source_input() {
    ...
    fin.open("./prog_frag/axi_commands_for_GRU64x64_8bits_1_timestep_4PEs.csv_input.json", ios::in);
...
SC_MODULE(testbench) {
  ...
  void run() {
    ...
    std::ofstream fout;
    fout.open("./test_output/test_output_gru.txt", ofstream::out | ofstream::trunc);
    flex_i.instr_log.open("./instr_log/instr_log_lstm.txt", ofstream::out | ofstream::trunc);
    ...
    while (undone) {
      ...
      //fout << "data in:" << '\t';
      //for (int k=0; k < 16; k++) {
      //  fout << hex << flex_i.data_signal[15-k] << ' ';
     // }
      //fout << endl;
    //  fout << "flex_i.status:" << '\t';
    ...
    fin.open("./flexnlp_results/flex_gru_64x64_result.txt", ios::in);
    std::stringstream fs;
    ...
    while(std::getline(fin, addr_str, ',')) {
        ...
        if (err>0) {
            ...
            fs << "ila2 data : ";
            for (int j = 0; j < 16; j++) {
                fs << hex << flex_i.flex_gb_core_large_buffer[addr_int + 15 - j] << " ";
            }
            fs << '\n';
        } else {
            fs << "PASSED!" << '\n';
