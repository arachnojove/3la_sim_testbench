...
//  void source_input();
  ...
  void source_input() {
    ...
    fin.open("./prog_frag/axi_commands_for_attention_ila2.csv_input.json", ios::in);
    json cmd_seq;
    fin >> cmd_seq;
...
SC_MODULE(testbench) {
  ...
  void run() {
    ...
    std::ofstream fout;
    fout.open("./test_output/test_output_attention.txt", ofstream::out | ofstream::trunc);
    flex_i.instr_log.open("./instr_log/attention_instr_log.txt", ofstream::out | ofstream::trunc);
    ...
    //  fout << "flex status:" << '\t';
    //  fout << "reduce valid: " << '\t' << flex.flex_gb_layer_reduce_is_valid << '\t';
    //  fout << "grouping num: " << '\t' << flex_i.flex_gb_layer_reduce_grouping_num << '\n' << endl;
      wait(10, SC_NS);
    }
    
    std::ifstream fin;
    fin.open("./flexnlp_results/flex_attention_result.txt", ios::in);
    std::stringstream fs;
    ...
    while(std::getline(fin, addr_str, ',')) {
      ...
      addr_str = addr_str.substr(4,6);
      addr_int = stoi(addr_str, nullptr, 16);
      std::getline(fin, data_str, '\n');
      
      // need to separate large buffer and small buffer
      if (addr_int < 0x600000) {
        addr_int = (addr_int) % 0x100000; 
        for (int j = 0; j < 16; j++) {
        ...
        for (int j = 0; j < 16; j++) {
            int index_ila = addr_int + 15 - j;
            int data_ila = flex_i.flex_gb_core_small_buffer[index_ila].to_int();
            int data_flex = stoi(data_str.substr(2+2*j, 2), nullptr, 16);
            if (data_ila == data_flex) {
                passed++;
            } else {
        ...
        if (err > 0) {
            ...
            fs << "ila2 data: ";
            for (int j = 0; j < 16; j++) {
                int data_ila = flex_i.flex_gb_core_small_buffer[addr_int + 15 - j].to_int();
                fs << std::hex << data_ila;
            }
            fs << '\n';
        } else {
