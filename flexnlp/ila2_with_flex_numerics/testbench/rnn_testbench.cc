...
//  void source_input();
  ...
  void source_input() {
    ...
    fin.open("./prog_frag/axi_commands_for_RNN64x64_8bits_1_timestep_4PEs.csv_input.json", ios::in);
    json cmd_seq; 
    fin >> cmd_seq;
...
SC_MODULE(testbench) {
  ...
  void run() {
    ...
    std::ofstream fout;
    fout.open("./test_output/test_output_rnn.txt", ofstream::out | ofstream::trunc);
    flex_i.instr_log.open("./instr_log/instr_log_rnn.txt", ofstream::out | ofstream::trunc);
    ...
    std::ifstream fin;
    fin.open("./flexnlp_results/flex_rnn_64x64_result.txt", ios::in);
    std::stringstream fs;
