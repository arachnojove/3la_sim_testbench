...
//  void source_input();
  ...
  void source_input() {
    ...
    fin.open("./prog_frag/axi_commands_for_maxpool.csv_input.json", ios::in);
...
SC_MODULE(testbench) {
  ...
  void run() {
    ...
    flex_i.instr_log.open("./instr_log/instr_log_maxp_flex.txt", ofstream::out | ofstream::trunc);
    ...
    //  fout << "grouping num: " << '\t' << flex.flex_gb_layer_reduce_grouping_num << '\n' << endl;
      wait(10, SC_NS);
    }
    
    std::ifstream fin;
    fin.open("./flexnlp_results/flex_maxp_result.txt", ios::in);
    std::stringstream fs;
